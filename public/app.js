...
function runDragAutoScroll() {
    ...
    if (dragAutoScrollY < threshold) {
        const intensity = 1 - (dragAutoScrollY / threshold)
        delta = -Math.max(4, Math.round(maxStep * intensity))
    } else if (dragAutoScrollY > window.innerHeight - threshold) {
        const intensity = 1 - ((window.innerHeight - dragAutoScrollY) / threshold)
        delta = Math.max(4, Math.round(maxStep * intensity))
    }
...
function renderContentPackDirs(dirs) {
    const normalized = normalizeRoots(dirs)
    contentPackDirsEl.innerHTML = ''

    if (normalized.length === 0) {
        const empty = document.createElement('div')
        empty.className = 'root-empty'
        empty.textContent = 'Пока не выбрано ни одной папки контента для объединения.'
        contentPackDirsEl.appendChild(empty)
    } else {
        normalized.forEach((dirPath, index) => {
            contentPackDirsEl.appendChild(createRootRow(dirPath, index))
        })
    }
...
async function chooseVmfPath() {
    try {
        ...
        if (selectedPath) {
            ...
            saveConfig()
            appendLog('info', '[pick]', 'VMF файл: ' + shorten(selectedPath, 120))
        }
    } catch (err) {
...
async function loadDefaults() {
    try {
        ...
        const initialConfig = {
            mode: initialMode,
            vmfPath: stored?.vmfPath || data.vmfPack?.vmfPath || '',
...
form.addEventListener('submit', async (ev) => {
    ...
    const runMsg = cfg.mode === 'content-pack'
        ...
        : cfg.mode === 'content-split'
            ...
            : `Запускаю упаковку ассетов VMF по ${cfg.contentRoots.length} папке(ам) контента...`
