...
export function getUnsafeOutputReason(outputPath: string, protectedPaths: ProtectedPath[]): string | null {
    ...
    const resolvedWorkspaceDir = path.resolve(DEFAULT_WORKSPACE_DIR)

    if (resolvedOutputPath === parsedOutputPath.root) {
        return `Нельзя очищать корень диска: ${resolvedOutputPath}`
    }

    if (resolvedOutputPath.toLowerCase() === resolvedWorkspaceDir.toLowerCase()) {
        return `Нельзя очищать корень workspace: ${resolvedOutputPath}`
    }

    if (!isSameOrInside(resolvedWorkspaceDir, resolvedOutputPath)) {
        return `Нельзя очищать путь вне workspace: ${resolvedOutputPath}. Для безопасности очистка разрешена только внутри ${resolvedWorkspaceDir}`
    }
