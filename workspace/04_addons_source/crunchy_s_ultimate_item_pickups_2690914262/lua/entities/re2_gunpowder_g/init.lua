...
function ENT:Initialize()
	self:SetModel( "models/crunchy/props/re3_props/re3_remake_explosives.mdl" )
...
function ENT:StartTouch(ent)
    if ent:GetClass() == "re2_gunpowder_g" and self.isCooking == false then
        ent:Remove()
...
function ENT:Think()
    if self.isCooking == true then
        if self.finishBakeTime <= CurTime() then
			self.isCooking = false
			self:Remove()
			local bread = ents.Create("re2_grenade_rounds")
