...
ENT.PrintName		= "Universal Ammo (6 Mags)"
...
function ENT:SpawnFunction(p,tr)
  local ent = ents.Create("universal_ammo_backpack")
...
function ENT:Initialize()
  self:SetModel("models/crunchy/props/l4d_props/pg_army_backpack.mdl")
...
function ENT:Touch(p)
  ...
  if(wep:GetPrimaryAmmoType() > -1) then
    ...
    if wep:GetMaxClip1() <= 0 then
      p:GiveAmmo(6,wep:GetPrimaryAmmoType())
    else
      p:GiveAmmo(wep:GetMaxClip1() * 6,wep:GetPrimaryAmmoType())
    end
    self:Remove()
  end
end
