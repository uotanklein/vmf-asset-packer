...
function ENT:Initialize()
	self:SetModel( "models/crunchy/props/re2_props/re2_remake_gunpowder_f.mdl" )
...
function ENT:StartTouch(ent)
    if ent:GetClass() == "re2_gunpowder_f" and self.isCooking == false then
        ent:Remove()
...
function ENT:Think()
    if self.isCooking == true then
        if self.finishBakeTime <= CurTime() then
			self.isCooking = false
			self:Remove()
			local bread = ents.Create("re2_sniper_ammo")
