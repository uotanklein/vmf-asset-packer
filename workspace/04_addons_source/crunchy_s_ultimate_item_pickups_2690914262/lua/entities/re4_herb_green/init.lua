...
function ENT:Initialize()
	self:SetModel( "models/crunchy/props/re4_props/herb.mdl" )
...
function ENT:Use(plyUse)
	...
	self:EmitSound("herb_pickup.wav")
	self:Remove(true)
	end
end

function ENT:StartTouch(ent)
    if ent:GetClass() == "re4_herb_green" and self.isCooking == false then
        ent:Remove()
...
function ENT:Think()
    if self.isCooking == true then
        if self.finishBakeTime <= CurTime() then
			self.isCooking = false
			self:Remove()
			local bread = ents.Create("re4_herb_green_mix")
