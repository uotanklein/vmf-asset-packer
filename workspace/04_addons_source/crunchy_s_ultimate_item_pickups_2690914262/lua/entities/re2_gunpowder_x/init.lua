...
function ENT:Initialize()
	self:SetModel( "models/crunchy/props/re8_props/re8_village_gunpowder.mdl" )
...
function ENT:StartTouch(ent)
    if ent:GetClass() == "re2_gunpowder_x" and self.isCooking == false then
        ent:Remove()
		self:EmitSound("gunpowder_mix_large.wav")
        self.isCooking = true
...
function ENT:Think()
    if self.isCooking == true then
        if self.finishBakeTime <= CurTime() then
			self.isCooking = false
			self:Remove()
			local bread = ents.Create("re2_ammo_pile")
