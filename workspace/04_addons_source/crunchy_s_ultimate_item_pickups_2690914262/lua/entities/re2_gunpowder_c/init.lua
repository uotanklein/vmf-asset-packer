...
function ENT:Initialize()
	self:SetModel( "models/crunchy/props/re2_props/re2_remake_high_grade_gunpowder.mdl" )
	self:PhysicsInit( SOLID_VPHYSICS )
	self:SetMoveType( MOVETYPE_VPHYSICS )
	self:SetSolid( SOLID_VPHYSICS )
	self.Entity:DrawShadow(true)
	self:SetModelScale(3.00)
...
function ENT:StartTouch(ent)
    if ent:GetClass() == "re2_gunpowder_c" and self.isCooking == false then
        ent:Remove()
...
function ENT:Think()
    if self.isCooking == true then
        if self.finishBakeTime <= CurTime() then
			self.isCooking = false
			self:Remove()
			local bread = ents.Create("re2_magnum_ammo_alt")
