...
function TOOL.BuildCPanel(DForm)
    ...
    e.OnChange = function()
        timer.Create("permall.saftyreset", 3, 1, function()
            RunConsoleCommand("permaall_secure1", 0)
        end)
    end
    ...
    e.DoClick = function()
        timer.Create("permall.saftyreset", 1, 1, function()
            RunConsoleCommand("permaall_secure1", 0)
        end)
...
function TOOL:DrawHUD()
    if not GetConVar("perma_all_crosshair_hud"):GetBool() then return end
    ...
    draw.RoundedBoxEx(h * .024, w * .525, h * .46, w * .1, h * .08, Color(0, 0, 0, 200), true, true, true, true)
    draw.SimpleTextOutlined("PERMANENT", "ixMediumFont", w * .535, h * .480, color_white, 0, 1, 2, Color(0, 0, 0, 100))
    draw.SimpleTextOutlined(class, "BudgetLabel", w * .535, h * .500, color_white, 0, 1, 2, Color(0, 0, 0, 100))
    draw.SimpleTextOutlined(s, "ixSmallFont", w * .535, h * .520, c, 0, 1, 2, Color(0, 0, 0, 100))
end
...
if CLIENT then
    concommand.Add("permaall_opentableeditor", function()
        ...
        DListView.OnRowRightClick = function(s, i, e)
            ...
            DMenu:AddOption("Copy UID").DoClick = function()
                CopyInfo(uid)
            end

            DMenu:AddOption("Copy Group UID").DoClick = function()
                CopyInfo(group_uid)
            end

            DMenu:AddOption("Copy Entity").DoClick = function()
                CopyInfo(entity)
            end

            DMenu:AddOption("Copy Entity (index)").DoClick = function()
                CopyInfo(entity:EntIndex())
            end

            DMenu:AddOption("Copy Class").DoClick = function()
                CopyInfo(class)
            end

            DMenu:AddOption("Copy Model").DoClick = function()
                CopyInfo(model)
            end
        ...
        DButton:SetSize(w - 20, 30)
        DButton:SetPos(10, h - 80)
        DButton:SetText("")
        ...
        local DButton = vgui.Create("DButton", DFrame)
        DButton:SetSize(w / 4, 30)
        DButton:SetPos(w - DButton:GetWide() - 10, h - 40)
        DButton:SetText("")
        ...
        local DButton = vgui.Create("DButton", DFrame)
        DButton:SetSize(w / 4, 30)
        DButton:SetPos(w / 2 - DButton:GetWide() / 2, h - 40)
        DButton:SetText("")
