...
function EFFECT:Init(data)
    ...
    -- (баг-фикс) Дульное устройство -- такая же клиентская модель, как WModel, и
    -- позиционируется только отрисовкой. Пока ствол не рисуют, она в начале
    -- координат, и вспышка со светом уходила в ЦЕНТР КАРТЫ. См.
    -- ARC9.AttachmentPosIsSane (arc9/common/sh_common.lua).
    if wm and !ARC9.AttachmentPosIsSane(pos, wpn:GetOwner()) then self:Remove() return end
