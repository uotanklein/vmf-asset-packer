...
function EFFECT:Init(data)
    ...
    -- (баг-фикс) Позиция берётся с КЛИЕНТСКОЙ модели оружия (ent.WModel[1]), а её
    -- двигает только отрисовка. Пока ствол не рисовали -- далёкий владелец, вне
    -- экрана, проход RT-прицела -- модель стоит в начале координат, и аттачмент
    -- отдаёт (0,0,0): гильзы сыпались в ЦЕНТР КАРТЫ. Подробно -- в
    -- ARC9.AttachmentPosIsSane (arc9/common/sh_common.lua).
    --
    -- Проверяем только мировую ветку: у вида от первого лица аттачмент снимается с
    -- вьюмодели, она всегда у камеры, и её координаты ещё пойдут через
    -- FormatViewModelAttachment. Промахнувшийся эффект убираем, а не двигаем
    -- наугад: оружие в этот момент всё равно не рисуется, показывать нечего.
    if !self.VMContext and !ARC9.AttachmentPosIsSane(origin, owner) then self:Remove() return end
