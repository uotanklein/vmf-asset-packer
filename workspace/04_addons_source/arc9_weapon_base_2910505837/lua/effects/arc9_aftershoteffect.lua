function EFFECT:Init(data)
	...
	if smoke and isstring(smoke) then
		...
		-- (баг-фикс) Дым цепляется к клиентской модели ствола, а её двигает только
		-- отрисовка: пока оружие не рисуют, модель в начале координат, и дым шёл в
		-- ЦЕНТР КАРТЫ. См. ARC9.AttachmentPosIsSane (arc9/common/sh_common.lua).
		if wm and !ARC9.AttachmentPosIsSane((muz or parent):GetPos(), wpn:GetOwner()) then
			self:Remove()
			return
		end
